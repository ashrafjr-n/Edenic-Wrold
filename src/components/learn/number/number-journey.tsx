...
export function NumberJourney({
  ...
  return (
    <div className="flex w-full flex-1 flex-col items-center gap-6 sm:gap-8">
      ...
      {/* One fixed-size stage for every step, so the numeral, the dotted
          guide and the drawing all land in exactly the same place and the
          page never jumps between steps. */}
      <div className="card anim-rise-in relative aspect-square w-full max-w-[17rem] p-5 sm:max-w-[20rem] sm:p-6">
        {stage === "watch" ? (
          <Image
            src={image}
      ...
      {stage === "watch" && videoId && (
        <div className="anim-fade-up flex w-full flex-col items-center gap-5">
          <NumberVideo
            videoId={videoId}
            posterImage={image}
            value={value}
          ...
          {/* The child moves on when they are ready. Detecting the end of the
              short would mean loading YouTube's IFrame API — another script
              from another origin on a children's page — to save one tap. */}
          <Button3D
      ...
      {stage !== "watch" && (
        <div className="anim-fade-up flex items-center gap-3 sm:gap-4">
          <Button3D
            ...
            onClick={retry}
            className="btn3d--clay-white px-6 py-3 text-sm text-[var(--color-ink)] sm:text-base"
          >
            <RotateCcw
